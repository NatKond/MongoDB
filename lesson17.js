...
db.restaurants.find({"cuisine": "Mexican"}, {menu: {$slice: -1}})
...
// Если удовлетворяют условию несколько документов, но мы выбрали updateOne, то изменения коснутся только первого документа
db.emp.updateOne(
    {department: 'HR'},
    {$inc: {commission: 10}}
)
...
// Выбрать сотрудников, у которых в массиве Skills 3 элемента
db.emp.find({skills: {$size: 3}});

// $exists выбирает только те документы, в которых определенный ключ присутствует или отсутствует.

// Выбрать сотрудников, у которых нет элемента с ключом company_adr
db.emp.find ({company_adr: {$exists:0}})

// $type извлекает только те документы, в которых определенный ключ имеет значение определенного типа

// Выбрать сотрудников, у которых элемент с ключом company_adr имеет тип string
db.emp.find ({department: {$type: "string"}})

// $regex задает регулярное выражение, которому должно соответствовать значение поля

// Выбрать сотрудников, у которых имя начинается на I
db.emp.find ({firstname: {$regex: "^I"}})

// Выбрать сотрудников, у которых фамилия заканчивается на "ova"
db.emp.find ({lastname: {$regex: 'ova$'}})

// Выбрать сотрудников, у которых в position есть слово "Hostess" (без учёта регистра)
db.emp.find({position: { $regex: 'hostess', $options: 'i'}})

// Выбрать всех, у кого в skills есть умение, содержащее "wine"
db.emp.find({skills: { $regex: 'wine', $options: 'i'}})

// Найти документы с email, заканчивающимся на @example.com
